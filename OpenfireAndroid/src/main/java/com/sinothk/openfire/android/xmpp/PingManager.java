...
//        super(connection);
//        executorService = Executors.newSingleThreadScheduledExecutor(new SmackExecutorThreadFactory(connection, "Ping"));
...
//
//        try {
//            Ping ping = new Ping(JidCreate.fromUnescaped(jid));
...
//        try {
//            return ServiceDiscoveryManager.getInstanceFor(connection()).supportsFeature(JidCreate.fromUnescaped(jid), Ping.NAMESPACE);
...
//        boolean res;
//        try {
//            res = ping(connection().getServiceName().toString(), pingTimeout);
