...
public class TextHolder extends RecyclerView.ViewHolder {
    ...
    public void doSomething(final IMMessage bean, String doctor_id, TextHolder holder) {
        if (IMConstant.FromType.SEND.equals(bean.getFromType())) {// 发送者id等于本人id,那么显示右边布局

            CommonUtils.setImage(view.getContext(), bean.getFromUserAvatar(), holder.chatImageviewRight);// 设置头像
        ...
        } else if (IMConstant.FromType.RECEIVE.equals(bean.getFromType())) {
            // 接受者id等于本人id,那么显示左边布局

            CommonUtils.setImage(view.getContext(), bean.getFromUserAvatar(), holder.chatImageviewLeft);// 设置头像
...
//
//            CommonUtils.setImage(view.getContext(), bean.getToUserAvatar(), holder.chatImageviewLeft);// 设置头像
//
//            holder.chatTimeTextviewLeft.setText(DateUtil.getFriendlyDate(new Date(bean.getMsgTime())));
//            holder.chatLinearlayoutLeft.setVisibility(View.VISIBLE);
//
